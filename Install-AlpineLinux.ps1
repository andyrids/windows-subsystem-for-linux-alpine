...
$Script:TaskDepth = 0
$Script:LastOutputInline = $false

$UnicodeSupport = [Console]::OutputEncoding.EncodingName -match "UTF-8" -or $Host.Name -match "Visual Studio Code" -or $PSVersionTable.PSVersion.Major -ge 7
...
function Invoke-Task {
    ...
    process {
        ...
        try {
            ...
            Show-TaskProgress -TaskName $Name -TaskResult FAIL -ToTitleCase:$ToTitleCase
...
Invoke-Task -Name "Fetching Alpine CDN information" -Critical -Steps @(
	{
		$CDNLinks = (Invoke-WebRequest -Uri $ALPINE_CDN -SkipHttpErrorCheck -UseBasicParsing).Links
...
Invoke-Task -Name "Updating indexes & upgrading packages" -Critical -Steps (
	{
		...
		if ($LASTEXITCODE -ne 0) { throw " - Failed to update & upgrade - $Log" }		
	}
)
# -----------------------------------------------------------------------------
...
Invoke-Task -Name "Updating Git config" -Steps @(
	{
		...
		if (Test-Path -Path $GCMPath -PathType Leaf) {
			...
			# Update skeleton config for cloud-init user generation
		...
		if  (-not ($GitUserName -and $GitUserEmail)) {
			throw "user.name & user.email are unset in WindowsGit config"
		}
...
Invoke-Task -Name "Awaiting cloud-init first-boot" -Critical -Steps @(
	{
		...
		$RecoverableErrors = $Status.recoverable_errors.ERROR | 
