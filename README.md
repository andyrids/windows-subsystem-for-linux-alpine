...
| doas cloud-init analyze blame                            | Display `cloud-init` report ordered by costly operations      |
| doas cloud-init analyze show                             | Display `cloud-init` report ordered by temporal cost          |
| doas cloud-init clean --logs --reboot                    | Remove `cloud-init` logs & re-run all stages on next boot     |
